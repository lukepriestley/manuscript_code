...
glm_hmm_dir <- "/Volumes/LaCie/cell_submission/data/behaviour/tus_experiment/glm_hmm/"
d$viterbi_state <- NA
n_states <- 2
...
fig4h <- ggplot(d %>%
         ...
         drop_na(ave_ev_z) %>%
         mutate(ave_ev_graph = cut(ave_ev_z, breaks = quantile(ave_ev_z, probs = seq(0, 1, length.out = 20), na.rm = T), labels = FALSE, include.lowest = TRUE)) %>%
         group_by(ave_ev_graph) %>%
         mutate(ave_ev_graph = mean(ave_ev_z)) %>%
         group_by(monkey, stim_cond, ave_ev_graph) %>% 
         summarise(m = mean(viterbi_state_bin, na.rm = T)), aes(x = ave_ev_graph, y = m, fill = stim_cond, color = stim_cond)) + 
  geom_point(size = 1.2, alpha = 0.50, position = position_jitter(width = 0.25, seed = 123)) + 
  geom_point(size = 1.8, shape = 1, alpha = 1, position = position_jitter(width = 0.25, seed = 123), color = 'black') + 
  geom_smooth(method = 'lm', linewidth = 0.65, alpha = 0.85, color = 'black') + 
  ...
  scale_color_manual(name = 'TUS condition', values = c('#FFF5F0', 'darkred'), labels = c('Sham', 'DRN')) + 
  scale_fill_manual(name = 'TUS condition', values = c('#FFF5F0', 'pink'), labels = c('Sham', 'DRN')) + 
  theme_pubr() + 
  theme(strip.text = element_blank(), text = element_text(size = 16), axis.text = element_text(size = 14), panel.spacing = unit(0.25, "lines"), aspect.ratio = 1, legend.position = c(0.15, 1.05), legend.direction = 'vertical', legend.title = element_blank()) + 
