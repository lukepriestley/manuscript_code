...
if(create_sim){
  ...
  hmm_path <- "/Volumes/LaCie/nature_neuro_submission/data/behaviour/glm_hmm_fits/" # path to fitted HMM objects
...
n_sim <- 10
write_dir <- "/Volumes/LaCie/nature_neuro_submission/data/behaviour/glm_hmm_param_recovery/"
...
simulation_path <- "/Volumes/LaCie/nature_neuro_submission/data/behaviour/glm_hmm_param_recovery/" # where simulated behaviour + model fits to simulated behaviour are located
...
ggplot(data = sim_log_lik, aes(x = factor(fit), y = log_lik, group = session_in_sim)) + 
...
##################### compare recovered vs true params #################
...
ggplot(d_tm %>% filter(sample > 0.87|sample < 0.14), aes(y = sample, group = simulation, fill = transition)) + 
...
figs3c <- ggplot(d_alpha, aes(y = alpha_value, x = hmm_state, fill = hmm_state, color = hmm_state, group = simulation)) + 
