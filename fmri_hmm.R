...
glm_hmm_dir <- "/Volumes/LaCie/nature_neuro_submission/data/behaviour/fmri_experiment/glm_hmm/"
d$viterbi_state <- NA
n_states <- 2
...
#write.csv(d, 'fmri_behaviour_with_glmhmm.csv') # write data for fMRI analysis
...
figure_data <- d %>% filter(monkey == 'Ulrich' & session == 6) %>% mutate(ev_norm = (ev)/3)
fig2a <- ggplot() +
  geom_line(data = figure_data, aes(x = trial, y = ev_norm), color = 'black', linewidth = 0.50) + 
  geom_point(data = figure_data, aes(x = trial, y = response_mod, shape = factor(response_mod)), position = position_jitter(height = 0), size = 2, alpha = 0.50, color = 'black') + 
  scale_shape_manual(name = 'Decision', labels = c('Reject', 'Pursue'), values=c(1, 8)) + 
...
################## FIGURE 3 #####################
################################################
# figure 3d: pursue-rates in decoded HMM states
fig3d <- ggplot(d %>%
  ...
  stat_summary(fun.data = 'mean_se', geom = 'pointrange', color = 'black', shape = 21, size = 0.75) + 
...
fig3e <- ggplot(d %>% drop_na(RT_z) %>%
                  group_by(monkey, session, viterbi_state) %>%
                  summarise(m = mean(RT_z, na.rm = T)),
                aes(x = factor(viterbi_state), y = m, fill = factor(viterbi_state), color = factor(viterbi_state))) + 
  geom_hline(yintercept = 0, linetype = 2, color = 'red') + 
  stat_summary(fun = 'mean', geom = 'bar', color = 'black', width = 0.60) + 
  geom_point(shape = 21, alpha = 0.50, size = 2.5, color = 'black') + 
...
# fig 3g: change in pursue-rates around state-transitions
...
fig3g <- ggplot(data = mean_se_data, aes(x = transition_time, y = mean_response*100, fill = transition_type, color = transition_type)) + 
...
# fig 3h: state occupancy as a function of availability of rewards (i.e. average expected value)
fig3h <- ggplot(d %>%
                  mutate(ave_ev_graph = cut(ave_ev_z, breaks = unique(quantile(ave_ev_z, probs = seq(0, 1, length.out = 10), na.rm = T)), labels = FALSE, include.lowest = TRUE)) %>%
                  group_by(ave_ev_graph) %>%
                  mutate(ave_ev_graph = mean(ave_ev_z)) %>%
                  group_by(monkey, session, ave_ev_graph) %>% 
                  summarise(m = mean(viterbi_state_bin, na.rm = T)), aes(x = ave_ev_graph, y = m, fill = m)) + 
  stat_summary(fun.data = 'mean_se', geom = 'pointrange', color = 'black', size = 0.75, shape = 21, fill = 'darkred') + 
  geom_smooth(method = 'lm', color = 'black', linewidth = 0.65, alpha = 0.50, fill = 'pink') + 
  geom_hline(yintercept = 0.5, linetype = 2, color = 'red') + 
  scale_y_continuous(name = 'P(high-motivation)', breaks = seq(0, 1, 0.10), limits = c(0, 1.0)) + 
