...
figure_data <- d %>% filter(monkey == 'Ulrich' & session == 3) %>% mutate(ev_norm = (ev)/3)
...
fig2a <- ggplot() +
  ...
  geom_point(data = figure_data, aes(x = trial, y = response_mod, shape = factor(response_mod)), position = position_jitter(height = 0), size = 2, alpha = 0.50, fill = 'darkgrey') + 
  geom_rect(data = viterbi_segments, aes(xmin = start_trial, xmax = end_trial, ymin = 0.05, ymax = 0.95, fill = as.factor(state)), alpha = 0.50) +
...
fig2jii <- ggplot(d %>%
                    group_by(monkey, session, richness_bin) %>%
                    summarise(m = mean(viterbi_state_bin, na.rm = T)), aes(x = richness_bin, y = m, fill = richness_bin)) + 
  geom_point() + 
  stat_summary(fun = 'mean', geom = 'bar', color = 'black') + 
  #stat_summary(fun.data = 'mean_se', geom = 'pointrange', color = 'black', size = 0.75, shape = 21) + 
  ...
  coord_cartesian(ylim = c(0.40, 0.8)) + 
  facet_grid(~monkey)
fig2jii
