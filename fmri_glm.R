...
################### GLMS ################
GLM1.1a <- glmer(response_mod ~ mag_z*prob_z + ave_rw_z + sd_ave_rw_z + trial_z + (ave_rw_z|monkey), data = d, family = 'binomial')
summary(GLM1.1a)

GLM1.1b <- glmer(response_mod ~ mag_z*prob_z + ave_rw_z + sd_ave_rw_z + trial_z + (sd_ave_rw_z|monkey), data = d, family = 'binomial')
summary(GLM1.1b)

GLM1.2 <- glmer(response_mod ~ mag_z*prob_z + prev_response_mod + trial_z + (prev_response_mod|monkey), data = d, family = 'binomial')
summary(GLM1.2)

GLM1.3 <- glmer(response_mod ~ mag_z*prob_z + ave_rw_z*prev_response_mod + trial_z + (ave_rw*prev_response_mod|monkey), data = d, family = 'binomial')
summary(GLM1.3)
...
fig1b <- ggplot(pdf_data_long, aes(x = x, y = pdf, fill = env)) + 
  ...
  theme(
    ...
    aspect.ratio = ASPECT_RATIO_STANDARD,
  )
fig1b

# Preprocessing for fig. 1c & fig. 1d
example_session <- d %>%
...
# Scale and pivot data for fig. 1d
scaled_data <- example_session %>%
  mutate(ave_rw = scale(ave_rw), sd_ave_rw = scale(sd_ave_rw)) %>%
  pivot_longer(cols = ends_with('_rw'), names_to = 'dimension', values_to = 'level')

# Plot for fig. 1d: Example session part 2
fig1d <- ggplot(scaled_data, aes(x = trial, y = level, color = dimension)) +
  geom_line(linewidth = LINE_WIDTH, alpha = 0.7) +
  theme_pubr() +
  labs(x = 'Trial', y = 'Environment [Z]') +
  scale_y_continuous(breaks = seq(-3, 3, 1.5), labels = number_format(accuracy = 0.01)) + 
  scale_color_manual(
    values = c('darkmagenta', 'skyblue2'),
    labels = c('Richness', 'Stochasticity'),
    name = 'Dimension'
  ) +
  theme(
    text = element_text(size = TEXT_SIZE_MAIN),
...
fig1e <- ggplot(
  figure_data %>%
    drop_na(ave_rw_ind) %>%
    mutate(ev_ind = cut(ev_z, breaks = unique(quantile(ev_z, probs = seq(0, 1, length.out = 10))), labels = FALSE, include.lowest = TRUE)) %>%
    group_by(ev_ind) %>%
...
fig1g <- ggplot(
  d %>%
    ...
    summarise(m = mean(response_mod, na.rm = TRUE) * 100),
  aes(x = factor(prev_response_mod), fill = factor(prev_response_mod), y = m)
) +
  stat_halfeye(position = position_dodge(width = 0.8), adjust = 1, justification = 0, .width = 0, point_colour = NA, alpha = ALPHA_LEVEL) +
  geom_boxplot(position = position_dodge(width = 0.8), width = 0.1, alpha = 0.60, outlier.color = NA) +
  geom_half_point(position = position_dodge(width = 0.8), side = "l", range_scale = .4, shape = 21, size = 1.5, alpha = ALPHA_LEVEL, color = 'black') +
  ...
  scale_fill_manual(name = 'Behavioural history', values = c('grey80', 'azure2'), labels = c('Reject', 'Pursue')) + 
  theme_pubr() + 
  theme(
    text = element_text(size = TEXT_SIZE_MAIN),
    axis.text = element_text(size = TEXT_SIZE_AXIS),
...
# Plot for fig. 1h: Behavioural-history * richness-of-environment
fig1h <- ggplot(
  d %>%
    drop_na(ave_rw_z) %>%
    mutate(
      ave_rw_ind = cut(ave_rw_z, breaks = unique(quantile(ave_rw_z, probs = seq(0, 1, length.out = 20))), labels = FALSE, include.lowest = TRUE)
    ) %>%
    group_by(ave_rw_ind) %>%
    mutate(ave_rw_graph = mean(ave_rw_z, na.rm = TRUE)) %>%
    ungroup() %>%
    group_by(monkey, ave_rw_graph, prev_response_mod) %>% 
    summarise(m = 100 * mean(response, na.rm = TRUE)),
  aes(x = ave_rw_graph, y = m, group = factor(prev_response_mod), fill = factor(prev_response_mod))
) +
  geom_point(size = 2.25, shape = 21, position = position_jitter(width = 0.1)) + 
  geom_smooth(method = 'lm', color = 'black', linewidth = LINE_WIDTH) + 
  scale_y_continuous(name = 'Pursue-rate (%)', limits = c(0, 100), breaks = seq(0, 100, 20)) + 
  scale_x_continuous(name = 'Env.richness [Z]', labels = number_format(accuracy = 0.1),  breaks = seq(-2, 2, 1)) + 
  scale_fill_manual(name = 'Behavioural history', values = c('#0b2370', '#B2D4FF'), labels = c('Reject', 'Pursue')) + 
  theme_pubr() + 
  theme(
    text = element_text(size = TEXT_SIZE_MAIN),
    axis.text = element_text(size = TEXT_SIZE_AXIS),
...
fig_s1a <- ggplot(S1A_data %>%
  ...
  stat_halfeye(position = position_dodge(width = 0.8), adjust = 1, justification = 0, .width = 0, point_colour = NA, alpha = 0.60) +
  geom_boxplot(position = position_dodge(width = 0.8), width = 0.1, alpha = 0.60, outlier.color = NA) +
  geom_half_point(position = position_dodge(width = 0.8), side = "l", range_scale = .4, shape = 21, size = 1, alpha = 0.50, color = 'black') +
  scale_y_continuous(name = 'Pursue-rate[%]', limits = c(20, 100), breaks = seq(20, 100, 20)) + 
...
# supplementary figure S1B: effect of individual rewards n-trials back
...
model_coefficients <- data.frame(monkey=character(),
                                 predictor=character(),
                                 t_stat=numeric(),
                                 se=numeric(),
                                 p_val=numeric())
...
for(m in monkeys){
  ...
  tmp_glm <- glm(response ~ 
                   ...
                   rw_9back + 
                   rw_10back +
                   trial_z, data = monkey_data, family = 'binomial')
  t_stat <- coef(tmp_glm)[4:13]
  se <- coef(summary(tmp_glm))[4:13,2]
  p_val <- coef(summary(tmp_glm))[4:13,4]
  model_coefficients <- rbind(model_coefficients, 
                              data.frame(monkey = rep(m, length(t_stat)),
                                         predictor = names(t_stat),
                                         se=as.vector(se),
...
model_coefficients$monkey<- factor(model_coefficients$monkey, levels = c('Ultra', 'Ulrich', 'Vampire', 'Winky'), labels = c('M1', 'M2', 'M3', 'M4'))
...
figs1b <- ggplot(model_coefficients %>% filter(predictor %in% c('t-1', 't-2', 't-3', 't-4', 't-5')), aes(x = predictor, y = t_stat, ymin = t_stat-1.96*se, ymax=t_stat+1.96*se, fill = monkey, color = monkey, group = monkey)) + 
  geom_line(alpha = 1) +
  geom_pointrange(alpha = 1, shape = 21, color = 'black') + 
  geom_hline(yintercept = 0, color = 'red', linetype = 2) + 
  scale_y_continuous(name = 'Effect-size [a.u.]') + 
  scale_x_discrete(name = 'Prev. reward') + 
  scale_fill_brewer(name = 'Monkey', palette = 'Blues') + 
  scale_color_brewer(name = 'Monkey', palette = 'Blues') + 
  theme_pubr() + 
...
figs1c <-  ggplot(d %>%
                   drop_na(ave_rw_z) %>%
                   mutate(ave_rw_ind = cut(ave_rw_z, breaks = unique(quantile(ave_rw_z, probs = seq(0, 1, length.out = 10))), labels = FALSE, include.lowest = TRUE)) %>%
                   group_by(ave_rw_ind) %>%
  ...
  geom_point(size = 2.25, shape = 21, alpha = 0.50, color = 'black', position = position_jitter(width = 0.1, height = 0.1), fill = 'blue4') + 
  geom_smooth(method = 'lm', linewidth = 0.65, color = 'black', fill = 'blue4') + 
  scale_y_continuous(name = 'Pursue-rate [%]', limits = c(0, 100), breaks = seq(0, 100, 20)) + 
...
figs1d <-  ggplot(d %>%
                   drop_na(sd_ave_rw_z) %>%
                   mutate(sd_ave_rw_ind = cut(sd_ave_rw_z, breaks = unique(quantile(sd_ave_rw_z, probs = seq(0, 1, length.out = 10))), labels = FALSE, include.lowest = TRUE)) %>%
  ...
  geom_point(size = 2.25, shape = 21, alpha = 0.50, color = 'black', position = position_jitter(width = 0.1), fill = '#6b0f2e') + 
  geom_smooth(method = 'lm', linewidth = 0.65, color = 'black', fill = '#6b0f2e') + 
  scale_y_continuous(name = 'Pursue-rate [%]', limits = c(0, 100), breaks = seq(0, 100, 20)) + 
